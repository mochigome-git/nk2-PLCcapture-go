...
func main() {
	...
	// Parse the device addresses for 16-bit devicessignalCh
	...
	go func() {
		for {
			...
			if !allDevicesRead(devices, devices32Parsed) {
				logger.Println("Not all devices are being read. Restarting the program...")
				restartProgram()
				break
			}
		}
...
func restartProgram() {
	...
	err := cmd.Start()
	if err != nil {
		log.Fatalf("Failed to start a new instance of the program: %v", err)
	}
}

// Function to check if all devices in devices32 are being read
func allDevicesRead(allDevices, devices32 []utils.Device) bool {
	for _, device := range devices32 {
		deviceRead := false

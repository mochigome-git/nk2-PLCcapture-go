...
func init() {
	err := godotenv.Load(".env.local.16")
	if err != nil {
...
func main() {
	...
	plcPort := getEnvAsInt16bit("PLC_PORT", 5011)
			...
			for _, device := range devices {
				for {
					...
					if err != nil {
						...
						continue // Retry after a delay if there is an error
					}
	...
	for i := 0; i < 10; i++ {
		go func() {
			for message := range dataCh {
				...
				topic := "testplc/holding_register/16bit/" + message["address"].(string)
...
func getEnv16bit(name, defaultValue string) string {
...
func getEnvAsInt16bit(name string, defaultValue int) int {
	if value, exists := os.LookupEnv(name); exists {
